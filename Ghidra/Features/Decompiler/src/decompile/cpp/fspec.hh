...
private:
  ...
  std::vector<int4> groupSet;	///< Group(s) \b this entry belongs to
...
public:
  ...
  const std::vector<int4> &getAllGroups(void) const { return groupSet; }	///< Get all group numbers \b this overlaps
...
public:
  ...
  virtual void assignMap(const std::vector<Datatype*>& proto, TypeFactory& typefactory, std::vector<ParameterPieces>& res,
...
protected:
  ...
  std::vector<int4> resourceStart;		///< The starting group for each resource section
  ...
  void separateSections(ParamActive *active,std::vector<int4> &trialStart) const;
